...
md"""
## ENGRI 1120 FINAL PROJECT: Design and Analysis of a Sustainable Cell-Free Production Process for Industrially Important Small Molecules
"""

# ╔═╡ 833d7250-f4ab-49a2-a43e-1b21def59ad4
html"""
<p style="font-size:20px;">Team name: SHEESH Team Members: Christopher Cheng, Patricia Coo, and Jack Chen </br>
...
md"""
### Introduction
Our team was challenged to utilize maltose in order to produce hydrazine at a 95% pure target molecule stream at a rate of about 1.0 grams per hour. 
	Hydrazine itself is vital in day to day life, found in multiple different forms, and utilized in many different ways. The most common use of all is “to make foaming agents like azodicarbonamide.”  The formation of azodicarbonamide is important because “it thermally decomposes to nitrogen, carbon dioxide, carbon monoxide, and ammonia.” This decomposition polymerizes and furthermore produces a “foamy plastic.” This “foamy plastic” is not only utilized to make the plastic products that we know so well, such as disposable cups, but it also aids in the creation of insulation within our homes and spandex which is worn daily. Not only is hydrazine used for this plastic material and the gases it gives off, it is also utilized in pesticides and pharmaceuticals proving its importance and versatility.  It can also be found within “fossil fuels and nuclear power stations.” Hydrazine and its explosive nature have also provided us with functioning air bags. One last known use of hydrazine is that it is used as rocket fuel propellant. When combined with dinitrogen tetroxide, this mixture becomes so explosive that no ignition is required. 
	
In order to produce our desired product of hydrazine, we must start by purchasing ammonia and nitrate, two compounds that will synthesize hydrazine when combined with the sugar feedstock of maltose. We will be utilizing Varnerlab’s well-mixed reactors to allow for this process to happen, and use separators to separate the products from the sugar. 
"""

# ╔═╡ 884a0a7d-e5d8-4417-b109-d00c37a01766
md"""
### Materials and Methods
First we determined the series of reactions needed to create hydrazine from maltose and determined that we needed to input maltose, ammonia, and nitrate into the reactor. Then we optimized the input stream by using the extent value and determined the amount of separators needed to reach a purity of 95%. Finally, we increased the number of chips to reach the minimum requirement of 1g/hr. For the financial aspect of this project, we determined the cost of all the equipment and looked up the cost of ammonia to run the system for a year on Sigma-Aldrich. Then we determine the amount of hydrazine we could create within a year of production and use the cost of the most expensive hydrazine we could find to see how much we can sell hydrazine for. Finally we input the data into the code to determine if the process is profitable.
"""
...
begin
	...
	# Let's lookup stuff that we want/need to supply to the chip to get the reaction to go -
	...
	mol_flow_values_feed_1 = [
		0.822 	; # nitrate mmol/hr 
		0.822 	; # ammonia mmol/hr 
	]
...
# ╔═╡ dccdb98e-0068-4a30-8da1-453e7ed1fb64
md"""
### Net Profit Value 
"""

# ╔═╡ b7d4ddf8-9aa1-4e3a-9954-b53bc062c100
# lifetime = 9 future years (period of time to payback the investment)
T = 9

# ╔═╡ 3ce634e3-9609-4d80-9af3-7c9f5a6dd1bc
...
# ╔═╡ f9bf0b9b-1cbe-4cf1-881a-6da84b37b68c
# Setup the cash flow array -
# we pay in year 1: $104.505K (out, -ve)
# we save in year(s) 2,3,4,5,6,7,8,and 9: -87.985K (in,+ve)
CF_array = [-104.505,-87.985,-87.985,-87.985,-87.985,-87.985,-87.985,-87.985,-87.985,-87.985]

# ╔═╡ 5449004b-1c10-4625-bc81-c452fcfd257d
...
# ╔═╡ d6185eb4-92e6-4842-8880-6837ca6adb4c
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ 637fa742-d15a-45a8-b745-85d19a934e6d
# what is the NPV?
NPV = sum(PV)

# ╔═╡ 8e0205fc-6904-469b-83c1-d7901c0c700e
md"""
### How do I interpret the NPV? 

Since the NPV is negative, the project would not be a good investment because the alternative of simply buying hydrazine is much cheaper. If one were to invest in our project they would be losing about $611,000 within 9 years from starting.
"""

# ╔═╡ fd339470-ffef-49fa-8636-dce7924e6405
md"""
### Conclusions
Using the financial balances and the NPV calculations, we have determined that our process would not be financially viable or favorable. One reason for this is that our process requires an extremely large number of reactor chips, each of which are $100, in addition to the syringe pump and magical separators. All of this equipment is necessary in order to produce hydrazine at a small amount of one gram per hour. Another reason for the poor financial performance of our process is that the raw materials, ammonia and nitrate, required for the process are also extremely expensive, and they are also required in large amounts. Although we can sell hydrazine for a moderately high value, this is still offset by the exorbitant cost of the equipment and raw materials necessary for this process. All of these factors contributed to an extremely negative net present value. Given these results, we cannot recommend that the client implements this process for the production of hydrazine. We must consider alternative approaches.
"""

# ╔═╡ 2f2713eb-a958-4d1a-a1cc-2723ea13c38c
md"""
### References
Magnificent Molecules: Hydrazine - https://edu.rsc.org/magnificent-molecules/hydrazine/2000023.article
